...
`generate-interventions` is the evidence-to-experiment generation layer: it reads an existing failure-intelligence run (its fingerprint, signal-source map, driver/dependency trace, and divergence graph) and emits a bounded set of explicit, reviewable intervention candidates as a manifest that feeds directly into `run-experiment-matrix`. It is generation-only — it never applies, executes, commits, or pushes anything, and never modifies the target repository:

```bash
rtl-agent generate-interventions \
  --failure-run .rtl-agent/runs/failure-001 \
  --repo ../axi-router \
  --allowed-file rtl/axi_pipe.sv \
  --max-candidates 8 \
  --output .rtl-agent/interventions/generated-001
```

Candidates are derived only from existing evidence via a small fixed template library — suppress an assignment, hold a register value, override one Boolean guard, and block one constant state transition (a time-windowed bounded-signal override is honestly recorded as unsupported by the current edit model rather than faked). Each candidate targets a divergent signal's driver (or a state-like enable in that driver's guard), carries a stable id, a hypothesis, exact cited evidence (source file/line/span, original text and hash, divergence node, driver statement, guard), an allowed-file list, and a deterministic evidence-completeness confidence (`high_evidence`, `moderate_evidence`, `low_evidence`, `insufficient_evidence` — reflecting how well the evidence grounds the edit, never the likelihood of fixing the bug). Ambiguous source spans, unresolved mappings, disallowed files, and spans that no longer match the target commit are refused rather than emitted; candidates are deduplicated by a semantic digest, deterministically ordered (strongest, earliest-divergence evidence first), and bounded by `--max-candidates`. The output directory holds a matrix-compatible `interventions.json`, a typed `intervention-templates.json`, a Markdown review report, and per-candidate unified diffs. There is no automatic application, no LLM-generated patches, no search or ranking by suspected root cause, and no causal claim — every candidate is an experiment proposal. The generated manifest is consumed unchanged by `rtl-agent run-experiment-matrix --interventions .rtl-agent/interventions/generated-001/interventions.json ...`. `scripts/intervention_templates_pilot_check.py` is a gated Icarus-backed pilot that generates candidates from the counterexample fixture's real evidence and confirms at least one drives a measured failure change through the matrix; it skips cleanly when the simulator is absent.
...
from rtl_agent.hkg import build_hkg, load_failure_bundle, write_graph

bundle = load_failure_bundle("failure-001", Path(".rtl-agent/runs/failure-001"))
graph = build_hkg([bundle], graph_id="debug-session-001")
...
Evidence-guided repair suggestions are deterministic inspect/review/check directions derived from existing counterfactual evidence, not generated patches. `generate_repair_suggestions(...)` consumes the original failure, generated intervention candidates, experiment outcomes, result comparisons, intervention rankings, and optional HKG memory. Each typed suggestion records the area to inspect, evidence basis, related source locations/signals, supporting interventions/outcomes, and an evidence-completeness confidence label. Suggestions are surfaced in the MVP demo JSON and Markdown under "Repair-direction suggestions". The rules prioritize ranked interventions with observed effects such as `failure_removed`, `failure_changed`, `new_failure`, or timing shifts; fall back to unranked altering outcomes when needed; and return no suggestions when only insufficient/no-observable evidence is available. They never generate code edits, add templates, or claim root cause.
