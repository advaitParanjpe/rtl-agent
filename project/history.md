...
## 2026-07-07 - Pre-HKG Architecture Review

Review/design-only milestone. Produced one focused architecture review artifact, `docs/architecture/pre-hkg-review.md`, before starting the Hardware Knowledge Graph. It documents the current service pipeline, the stable schema-versioned input/output contracts (with the `RunArtifact` id + sha256 + schema-version provenance triple as the ingestion contract), the main report/model types, the reusable failure-intelligence run evidence artifacts, where the HKG should plug in (a new read-only `hkg` module ingesting already-produced runs/reports, keyed on the canonical fingerprint), what HKG v0 should store (canonical failure-identity nodes, signal/source-location nodes, driver/dependency edges, cluster membership, intervention→observed-effect records, provenance pointers) and explicitly not store yet (causal edges, semantic RTL elaboration, raw blobs, cross-run inference, cross-failure ranking, embeddings, database/server/UI), and the risks/technical debt to resolve first. No code was changed. Validation was limited to `git diff --check` on the single touched Markdown file; no full regression was needed because no code changed (the doc lives in `docs/` and is not scraped by the README-based CLI-inventory or packaging-smoke checks).

Recommended HKG v0 scope: a read-only `hkg` module that ingests validated failure-intelligence runs and typed reports and materializes a typed, versioned, deterministic file-based graph artifact (canonical identity nodes, structural signal/source/driver edges, cluster membership, intervention→observed-effect records), reusing `fingerprint_run` / `member_from_fingerprint` / `cluster_failures` / `ExperimentComparison` / `InterventionRanking`, storing references + hashes rather than copies, and adding no new analysis or causal claim.

Blockers to resolve before HKG implementation: (1) merge the unmerged milestone chain carrying stages 53-59 (canonical fingerprint, clustering, ranking, comparison) to `main` in order, since the HKG builds directly on them; (2) standardize on the canonical clustering (`failure_clustering`) so the HKG ingests one notion of cluster rather than also the older family clustering; (3) confirm HKG v0 persists a deterministic file-based graph artifact (no database) to stay within the established read-only, deterministic, hash-cited convention.

## 2026-07-07 - Hardware Knowledge Graph v0 - Graph Construction

Built the first deterministic Hardware Knowledge Graph construction layer. Added a new `rtl_agent.hkg` module with typed, versioned graph models and a builder that ingests only existing structured evidence models: validated failure-intelligence run artifacts, failure fingerprints, optional experiment-matrix and intervention-template reports, optional experiment comparisons and intervention rankings, and optional canonical failure-clustering reports. It emits a deterministic JSON graph artifact via `write_graph`.

The graph vocabulary is intentionally bounded to the v0 construction scope. Node types: Module, Signal, SourceLocation, Failure, CanonicalFingerprint, FailureCluster, Intervention, Experiment, and ObservedEffect. Edge types: contains, drives, depends_on, originated_from, belongs_to_cluster, generated, produced, and references. Every node and edge records provenance to the originating artifact (`artifact_id`, schema version, content SHA-256/path where available). Construction uses stable node/edge ids, deterministic input ordering, sorted attributes/provenance, and sorted serialized node/edge lists.
