...
21. **Waveform Evidence Bundle Integration** - Active next milestone: teach the deterministic evidence-bundle exporter to recognize, classify, hash, and record schema versions for waveform-slice and assertion-link artifacts found under a run directory, with provenance and omitted-content handling, without redesigning export or changing existing artifact schemas.
22. **Prohibited-Shortcut Review Finding Example Check** - Deferred: add a compact local example check exercising the existing but currently untested `det-prohibited-shortcut-N` review finding, using a deliberate diff that textually conflicts with a task-contract prohibited shortcut. Remains intentionally deferred.

## Current Status

Stages 1 through 20 are complete. Stage 21 is active. Stage 22 is deferred.
