...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="rtl-agent-no-change-example-") as raw_tmp:
        ...
        assert any(record.path == "rtl/top.sv" for record in repository_map.files)

        run_cli(
            [
                "parse-issue",
                "--issue",
                str(examples / "issues" / "reset-behavior.md"),
                "--repository-map",
                str(repository_map_path),
                "--output",
        ...
        assert [item.value for item in task_contract.scoped_repository_context] == [
            "rtl/top.sv",
            "tb/top_tb.sv",
        ]
        assert len(task_contract.acceptance_criteria) == 2

        top_sv = examples / "simple-rtl" / "rtl" / "top.sv"
        original_top_sv = top_sv.read_text(encoding="utf-8")
        ...
        implementation_summary = run_cli(
            [
                ...
                str(repository_map_path),
                "--provider-plan",
                str(examples / "provider-plans" / "no-change.json"),
                "--allowed-file",
                "rtl/top.sv",
                "--max-iterations",
                "1",
        ...
        assert implementation_summary["status"] == "proposed_diff"
        assert implementation_summary["failure_reason"] is None
        assert implementation_summary["applied_files"] == ["rtl/top.sv"]
        ...
        assert implementation_report.applied_files == ["rtl/top.sv"]
        ...
        assert tool_result.path == "rtl/top.sv"
        assert tool_result.status == "applied"
        assert implementation_report.diff_path is not None
        assert implementation_report.diff_path.exists()
        assert implementation_report.diff_path.read_text(encoding="utf-8") == ""
        assert top_sv.read_text(encoding="utf-8") == original_top_sv
        ...
        assert review_report.checked_files == ["rtl/top.sv"]
        ...
        assert verification_strength.changed_files == ["rtl/top.sv"]
        ...
        assert "implementation/diff.patch" in artifacts_by_path
    ...
    print("compact no-change example check passed")
    return 0
